...
std::unordered_map<std::string, std::string> GetConfigFileValues(
  ...
  while (std::getline(config_file, line)) {
    ...
    std::string key = TrimString(line.substr(0, split_position));
    std::string value = TrimString(line.substr(split_position + 1));
...
void ReadNextValue(
  ...
  if (!data_file->read(next_bytes, data_size)){
...
void HSIDataOptions::ReadHeaderFromFile(const std::string& header_file_path) {
  ...
  itr = header_values.find("spectra names");
  if (itr != header_values.end()) {
    specNames = itr->second;
    std::cout << "spectra names = " << specNames.substr(0,50) << "..." << std::endl;
  }

  itr = header_values.find("wavelength");
  if (itr != header_values.end()) {
    wavelength = itr->second;
    std::cout << "wavelength = " << wavelength.substr(0,50) << "..." << std::endl;
  }

  itr = header_values.find("bbl");
  if (itr != header_values.end()) {
    bbl = itr->second;
    std::cout << "bbl = " << bbl.substr(0,50) << "." << std::endl;
  }

  itr = header_values.find("fwhm");
  if (itr != header_values.end()) {
    fwhm = itr->second;
    std::cout << "fwhm = " << fwhm.substr(0,50)<< "..." << std::endl; 
  }
...
HSIDataValue HSIData::GetValueLib(
    const int row, const int col, const int band) const {
  ...
  if (interleave_format == HSI_INTERLEAVE_BSQ) {
    ...
    const int band_index = (num_rows * num_cols) * band;
    //index = band_index + (row * num_cols) + col;
    index = band_index + (col * num_cols) + row;
  } else if (interleave_format == HSI_INTERLEAVE_BIL) {
    // BIL: row > band > col.
    const int row_index = (num_cols * num_bands) * row;
    index = row_index + (band * num_cols) + col;
...
void HSIDataReader::ReadData(const HSIDataRange& data_range) {
  ...
  //--orig defn-- std::ifstream data_file(data_options_.hsi_file_path);
  std::ifstream data_file(data_options_.hsi_file_path, std::ios::in | std::ios::binary);
